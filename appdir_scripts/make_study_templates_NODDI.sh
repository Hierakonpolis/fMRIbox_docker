...
then
  cd /out/template/bc
  for anat_bc in /out/*/anat/*_bc.nii.gz; do
    subj=$(basename "$(dirname "$(dirname "$anat_bc")")")
    ln "$anat_bc" "./${subj}_bc.nii.gz"
  done
  buildtemplateparallel.sh -c 2 -j $(nproc) -n 0 -r 1 -d 3 -o T1w_bc_ *_bc.nii.gz
  rm *_bc.nii.gz
